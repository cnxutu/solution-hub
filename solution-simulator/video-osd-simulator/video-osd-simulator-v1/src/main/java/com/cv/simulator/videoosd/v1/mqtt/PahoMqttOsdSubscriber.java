...
public class PahoMqttOsdSubscriber implements MqttOsdSubscriber {
    ...
    @Override
    public MqttOsdSubscriberSession subscribe(final MqttOsdProperties properties, final MqttOsdMessageHandler handler) {
        validate(properties);
        try {
            final String clientId = hasText(properties.getClientId())
            ...
            final MqttClient client = new MqttClient(properties.getBrokerUrl(), clientId, new MemoryPersistence());
            ...
            return new MqttOsdSubscriberSession() {
                @Override
                public void close() {
                    closeClient(client, properties.getTopic());
                }
            };
