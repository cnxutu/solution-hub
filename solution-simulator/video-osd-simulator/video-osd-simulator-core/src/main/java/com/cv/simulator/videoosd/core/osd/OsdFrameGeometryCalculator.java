...
public class OsdFrameGeometryCalculator {
    ...
    public void populateFrameGeometry(DeviceTelemetryRecord record, double hfovDeg, double vfovDeg) {
        if (record == null || record.getLatitude() == null || record.getLongitude() == null
                || record.getHeight() == null || record.getHeight() <= 0F) {
            return;
        }
        ...
        double headingDeg = record.getAttitudeHead() == null ? 0D : record.getAttitudeHead();
        double halfWidthMeters = record.getHeight() * Math.tan(Math.toRadians(hfovDeg / 2.0D));
        double halfHeightMeters = record.getHeight() * Math.tan(Math.toRadians(vfovDeg / 2.0D));
