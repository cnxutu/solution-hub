...
VALUES ('demo-rtmp-task', './sample-videos', './sample-videos/demo.mp4', '*.mp4,*.flv', '127.0.0.1', 7935, 'live', 'drone001', 'RTMP', 'webrtc-pusher --input "{videoFile}" --host {zlmHost} --app {app} --stream {stream}', TRUE, '-re', DATEADD('MINUTE', -5, CURRENT_TIMESTAMP), DATEADD('MINUTE', 5, CURRENT_TIMESTAMP), 'CREATED', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, 0);
